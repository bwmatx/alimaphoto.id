...
function handleCompleteQuery(message, chatState) {
    ...
    if (/^complete\s*1$|^complete satu$|^complete1$|^4[\.\,]\s*1$|^4\.1$|^nomor 4\.1$/.test(msg)) {
        return {
            ...
            step: 'benefit',
            chatState: { lastIntent: 'package_detail', lastTierMentioned: 'Complete 1', lastOptions: [], lastQuestionType: null, selectedPackage: 'complete_1' }
        };
    }
    ...
    if (/^complete\s*2$|^complete dua$|^complete2$|^4[\.\,]\s*2$|^4\.2$|^nomor 4\.2$/.test(msg)) {
        return {
            ...
            step: 'benefit',
            chatState: { lastIntent: 'package_detail', lastTierMentioned: 'Complete 2', lastOptions: [], lastQuestionType: null, selectedPackage: 'complete_2' }
        };
    }
    ...
    if (/^complete\s*3$|^complete tiga$|^complete3$|^4[\.\,]\s*3$|^4\.3$|^nomor 4\.3$/.test(msg)) {
        return {
            ...
            step: 'benefit',
            chatState: { lastIntent: 'package_detail', lastTierMentioned: 'Complete 3', lastOptions: [], lastQuestionType: null, selectedPackage: 'complete_3' }
        };
    }
...
app.post('/api/chat', function (req, res) {
    try {
        ...
        if (!apiKey) {
            return res.status(500).json({ error: true, message: 'Konfigurasi server belum lengkap. API key tidak ditemukan.' });
        }

        var message = req.body.message;
        var chatState = req.body.chatState || {};

        if (!message || typeof message !== 'string' || message.trim() === '') {
            return res.status(400).json({ error: true, message: 'Pesan tidak boleh kosong.' });
        }
        if (message.length > 500) {
            return res.status(400).json({ error: true, message: 'Pesan terlalu panjang (maks 500 karakter).' });
        }
        ...
        try {
            ...
            if (completeResult) {
                return res.json({ reply: completeResult.reply, step: completeResult.step, chatState: completeResult.chatState });
            }

            var categoryResult = handleCategoryQuery(normalizedMsg);
            if (categoryResult) {
                return res.json({ reply: categoryResult.reply, step: categoryResult.step, chatState: categoryResult.chatState });
            }

            var numericResult = handleNumericCategorySelect(normalizedMsg, chatState);
            if (numericResult) {
                return res.json({ reply: numericResult.reply, step: numericResult.step, chatState: numericResult.chatState });
            }

            var scopedTierResult = handleScopedTierSelection(normalizedMsg, chatState);
            if (scopedTierResult) {
                return res.json({ reply: scopedTierResult.reply, step: scopedTierResult.step, chatState: scopedTierResult.chatState });
            }
        } catch (handlerErr) {
        ...
        fetch(baseUrl + '/chat/completions', {
        ...
        }).then(function (deepseekRes) {
            if (!deepseekRes.ok) {
                throw new Error('DeepSeek API status ' + deepseekRes.status);
            }
            return deepseekRes.json();
        }).then(function (data) {
            ...
            return res.json({ reply: reply, step: detectStep(reply), chatState: newChatState });
        }).catch(function () {
            return res.status(500).json({ error: true, message: 'Maaf, terjadi kendala saat memproses pertanyaan Anda.' });
        });
...
function buildSystemPrompt() {
    return 'Kamu adalah Min Limpo, asisten informasi Alima Photo.\n' +
        ...
        '- Gunakan emoji HANYA 🙂 dan 🙏 jika perlu, seperlunya saja.\n' +
        ...
        '"Untuk detail gear yang digunakan tim Alima Photo, boleh langsung chat Min Limpo via WhatsApp ya kak 🙏\n' +
        'Nomor WhatsApp: 0852 3232 1080"\n' +
        '\n' +
        '=== WAKTU PENGERJAAN ===\n' +
        'Jika user tanya estimasi pengerjaan, jawab:\n' +
        '"Untuk semua paket, estimasi pengerjaan maksimal 1 bulan setelah acara atau setelah pelunasan ya kak 🙏\n' +
...
app.listen(PORT, function () {
    console.log('Alima chatbot backend running on http://localhost:' + PORT);
});
